...
vi.mock('#imports', () => {
  ...
  const createItem = (key) => {
    ...
    let value = key.includes('panelVisible') ? false : (key.includes('hasAnalysisData') ? false : null);
...
describe('StorageService Lifecycle Management', () => {
  ...
  describe('Storage Watchers', () => {
    it('should properly register and cleanup watchers', async () => {
      ...
      expect(watchCallback).toHaveBeenCalledWith(expect.objectContaining({
        isPanelVisible: true
      }));
  ...
  describe('Storage Quota and Performance', () => {
    it('should handle large analysis results without exceeding storage limits', async () => {
      ...
      const largeResult = {
        ...
        strengths: Array.from({ length: 50 }, (_, i) => `Strength ${i}: Lorem ipsum dolor sit amet`),
        recommendations: Array.from({ length: 50 }, (_, i) => `Recommendation ${i}: Lorem ipsum dolor sit amet`),
      };
