/**
 * Export a type for all available icon names
 * This allows for proper typing without exposing the private ICON_PATHS
 */
...
export class IconHelper {
  // SVG path definitions for various icons (kept private)
   ...
   * @returns SVG element
   */
  public static createSvgIcon(iconName: string, viewBox = '0 0 16 16'): SVGElement {
    ...
    // Get the path data safely
    const pathData = this.getIconPath(iconName);
   ...
   * @returns SVG string
   */
  public static getSvgIconString(iconName: string, viewBox = '0 0 16 16'): string {
    // Get the path data safely
    const pathData = this.getIconPath(iconName);

    // Create a properly formatted SVG string with xmlns attribute
    return `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="${viewBox}" fill="currentColor"><path d="${pathData}"></path></svg>`;
  }
  ...
  /**
   * Get the path data for a specific icon
  ...
  private static getIconPath(iconName: string): string {
    ...
    if (!this.hasIcon(iconName)) {
      ...
      // Return a default circle path
      return 'M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14z';
    }

    // Access the path data safely
    return this.ICON_PATHS[iconName as keyof typeof this.ICON_PATHS];
  }
}
