...
export class GitHubAPI {
  ...
  public async fetchEndpoint<T>(endpoint: string): Promise<T> {
    try {
      ...
      const response = await fetch(`https://api.github.com/repos/${this.username}/${this.repoName}${endpoint}`, {
        headers: headers
      });
      ...
      if (response.status === 403) {
        ...
        if (rateLimitRemaining === '0') {
          ...
          const minutesUntilReset = Math.round((resetDate.getTime() - new Date().getTime()) / 60000);
          
          throw new Error(`GitHub API rate limit exceeded. The limit will reset in approximately ${minutesUntilReset} minutes.`);
        }
      }
      ...
      console.error(`Error in API request: ${error instanceof Error ? error.message : 'Unknown error'}`);
      throw error;
    }
  }
